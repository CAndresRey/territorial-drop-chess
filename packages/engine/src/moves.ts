...
const validateMove = (
  ...
  if (
    piece.position.x !== action.from.x ||
  ...
  const isLegal = legalMoves.some(
    (c) => c.x === action.to.x && c.y === action.to.y,
  );
...
const validateDrop = (
  action: Extract<PlayerAction, { type: 'drop' }>,
  state: GameState,
): ValidationResult => {
  ...
  if (playerCount >= 5) {
    ...
    let lastRoundWithDrop:
      | { round: number; actions: PlayerAction[] }
      | undefined;
    ...
    const isOwnTerritory = player.territory?.squares.some(
      (s) => s.x === action.to.x && s.y === action.to.y,
    );
    const isOtherTerritory = Object.values(state.players).some(
      (p) =>
        p.id !== action.playerId &&
        p.territory?.squares.some(
          (s) => s.x === action.to.x && s.y === action.to.y,
        ),
    );
...
const applyMove = (
  ...
  if (capturedIdx !== -1) {
    ...
    if (captured.type === PieceType.King) {
      ...
      const reserveType =
        captured.type === PieceType.Veteran ? PieceType.Pawn : captured.type;
