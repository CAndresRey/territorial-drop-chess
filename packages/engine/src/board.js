import { DEFAULT_FORMATION_TEMPLATES, FORMATION_PIECE_COUNT, validateFormationSelection, } from './formation.js';
import { PieceType, } from './types.js';
const createPiece = (nextId, owner, type, x, y) => ({
    id: nextId(),
    owner,
    type,
    position: { x, y },
});
const getBoardSizeForPlayerCount = (playerCount) => {
    if (playerCount === 2)
        return 11;
    if (playerCount <= 4)
        return 13;
...
const validateCreateGameInput = (config, playerIds) => {
...
const getTerritory = (playerId, index, playerCount, boardSize) => {
    const squares = [];
    let palaceOrigin = { x: 0, y: 0 };
    const palaceSize = 3;
    if (playerCount === 2) {
...
const getStartingArmy = (nextId, playerId, index, playerCount, boardSize) => {
    const army = [];
    if (playerCount === 2) {
        ...
        const getX = (x) => (index === 0 ? x : boardSize - 1 - x);
        army.push(createPiece(nextId, playerId, PieceType.Rook, getX(startX), yBase));
        army.push(createPiece(nextId, playerId, PieceType.Knight, getX(startX + 1), yBase));
        army.push(createPiece(nextId, playerId, PieceType.King, getX(startX + 2), yBase));
        army.push(createPiece(nextId, playerId, PieceType.Bishop, getX(startX + 3), yBase));
        for (let i = 0; i < 4; i++) {
            army.push(createPiece(nextId, playerId, PieceType.Pawn, getX(startX + i), yPawn));
        }
        return army;
    }
    if (playerCount <= 4) {
        ...
        if (index === 0) {
            // NW (P1)
            [0, 1, 2, 3].forEach((x) => army.push(createPiece(nextId, playerId, PieceType.Pawn, x, pawnYTop)));
            ...
            // NE (P2)
            [max - 3, max - 2, max - 1, max].forEach((x) => army.push(createPiece(nextId, playerId, PieceType.Pawn, x, pawnYTop)));
            army.push(createPiece(nextId, playerId, PieceType.Pawn, max - 3, pawnYTop + 1));
            army.push(createPiece(nextId, playerId, PieceType.Guard, max - 2, royalYTop));
            army.push(createPiece(nextId, playerId, PieceType.King, max - 1, royalYTop));
            army.push(createPiece(nextId, playerId, PieceType.Bishop, max - 2, backYTop));
            army.push(createPiece(nextId, playerId, PieceType.Knight, max - 1, backYTop));
            ...
            army.push(createPiece(nextId, playerId, PieceType.Pawn, max - 3, pawnYBottom - 1));
            [max - 3, max - 2, max - 1, max].forEach((x) => army.push(createPiece(nextId, playerId, PieceType.Pawn, x, pawnYBottom)));
        }
        else if (index === 3) {
            // SW (P4)
            ...
            army.push(createPiece(nextId, playerId, PieceType.Pawn, 3, pawnYBottom - 1));
            [0, 1, 2, 3].forEach((x) => army.push(createPiece(nextId, playerId, PieceType.Pawn, x, pawnYBottom)));
        }
        return army;
    }
    ...
    if (index < 4)
        return getStartingArmy(nextId, playerId, index, 4, boardSize);
    // Edge setups (simplified but matching zones)
    if (index === 4) {
        // N (F13-J15)
        [5, 6, 7, 8, 9].forEach((x) => army.push(createPiece(nextId, playerId, PieceType.Pawn, x, 13)));
        ...
        // E (L6-O10)
        [11, 11, 11, 11, 11].forEach((x, i) => army.push(createPiece(nextId, playerId, PieceType.Pawn, x, 5 + i)));
        ...
        // S (F1-J3)
        [5, 6, 7, 8, 9].forEach((x) => army.push(createPiece(nextId, playerId, PieceType.Pawn, x, 1)));
        ...
        // W (A6-D10)
        [3, 3, 3, 3, 3].forEach((x, i) => army.push(createPiece(nextId, playerId, PieceType.Pawn, x, 5 + i)));
...
const getSlotCoordinates = (playerId, index, playerCount, boardSize) => getStartingArmy(() => 'slot', playerId, index, playerCount, boardSize).map((piece) => piece.position);
const buildArmyFromFormation = (nextId, playerId, slots, template) => {
    if (slots.length !== FORMATION_PIECE_COUNT) {
        throw new Error(`Formation mode requires ${FORMATION_PIECE_COUNT} slots, got ${slots.length}`);
    }
    return template.pieces.map((pieceType, idx) => createPiece(nextId, playerId, pieceType, slots[idx].x, slots[idx].y));
};
// ── Public API ───────────────────────────────────────────────────────────────
export const createGame = (config, playerIds, options) => {
    ...
    const selectedFormations = formationEnabled
        ? validateFormationSelection(playerIds, formationTemplates, options?.formationSelections, normalizedConfig.formation?.required ?? true)
        : null;
    const colors = [
        '#FF0000',
        '#0000FF',
    ...
    playerIds.forEach((id, index) => {
        const territory = getTerritory(id, index, normalizedConfig.playerCount, normalizedConfig.boardSize);
        players[id] = {
            id,
            score: 0,
            isEliminated: false,
        ...
        if (formationEnabled) {
            const slots = getSlotCoordinates(id, index, normalizedConfig.playerCount, normalizedConfig.boardSize);
            const selected = selectedFormations[id];
            pieces.push(...buildArmyFromFormation(nextId, id, slots, selected));
        }
        else {
            const army = getStartingArmy(nextId, id, index, normalizedConfig.playerCount, normalizedConfig.boardSize);
            pieces.push(...army);
        }
    });
    return {
